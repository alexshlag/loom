...
## 📍 Roadmap & Project Plan (Development)
...
* Коммит **по запросу пользователя**: `git add -A; git commit -m "<type>: <desc>"`
  - **Commit format**: `<type>: <description>` (type: feat|fix|refactor|schema|lint/ingest/query)
...
#### Assets & Media Pipeline (`assets/images/`, `assets/descriptions/`)

Две связанные поддиректории для работы с изображениями:

| Поддиректория | Назначение | Формат |
|---------------|------------|--------|
| `assets/images/` | Копии оригиналов изображений (.png, .jpg, .jpeg, .gif) | Бинарные файлы |
| `assets/descriptions/` | Markdown-описания с OCR, entities и metadata | `.md` файлы |

**Workflow для ingest изображения:**
1. Изображение копируется в `raw/` через capture flow
2. Агент извлекает: OCR текст, определяет entities на изображении, генерирует metadata
3. Описание сохраняется как `.md` в `assets/descriptions/` с именем, соответствующим оригиналу изображения
4. Ссылка на изображение добавляется в wiki-страницу через wikilink: `![[assets/images/filename.png]]`
...
* **Приоритет в поиске**: syntheses/ → concepts/ → entities/ (FAQ-слоё ищет в первую очередь)
...
**Trigger**: ≥3 related sources detected or user provides multiple files.
**Flow**: `scripts/batch-ingest.sh --scan` → clusters → present to user for approval.
