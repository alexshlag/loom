...
Priority chain: `index_lookup → semantic_search → ./scripts/wiki-search.sh (--fallback full grep)`

> Full flow in `process-query.json#search_priority_details`. Wiki-search uses dynamic category prioritization (syntheses→concepts→entities→...).
...
| Contradiction resolution | `AGENTS.md#contradiction-resolution → authoritative > temporal > user_review` |
...
`./scripts/rebuild-meta.sh` → rebuilds `meta/registry.json` + `meta/backlinks.json`

**Trigger points**: After any wiki edit in Ingest / Query / Lint processes.

> Full integration flow: `process-ingest.json#step_3a`, `process-query.json#post_check`, `process-lint.json#check_id_7`}
