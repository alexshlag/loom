...
## 🧠 Context Management — Persistent vs Transient Rules (Phase 31)

### Why This Matters

AGENTS.md и process-файлы (`process-ingest.json`, `process-query.json`, `process-lint.json`) вместе занимают ~86KB. Это избыточный контекст сессии. Агент читает ВСЕ правила при старте, но многие из них нужны только на время конкретного процесса (ingest/query/lint).

**Проблема**: Agent сохраняет в контексте все process-specific rules даже после завершения процесса → bloat memory.

**Решение**: Разделить правила на три scope:
- **Persistent** — помнить всю сессию (memory model, execution contract, error handling)
- **Transient** — читать только во время процесса, забыть после (process-specific steps)
- **Hybrid** — частично помнить, но читать из source only when needed
...
Agent читает `rules/context-scopes.json` при старте сессии → определяет какие правила persistent, какие transient.

**Persistent rules**: Всегда в контексте. Read once at session start, never forget during session.

**Transient rules**: Read when entering process mode, forget after process completes. Never keep in context unless user explicitly asks.

**Hybrid rules**: Remember but don't keep full content in context. Read source file only when actively working on that topic.

### Context Management Rules

1. **Session start**: Read persistent rules from AGENTS.md → these are always in context.
2. **Process start (ingest/query/lint)**: Read process-specific transient rules ONLY for active process.
3. **Process complete**: Forget transient rules unless user says "keep this".
4. **Hybrid rules**: Don't keep full content in context. Reference source file only when needed.

### Schema Reference

- Full scope definitions: `rules/context-scopes.json`
- Scope metadata added to each process file via `scope: transient | persistent | hybrid`

> Canonical: `AGENTS.md#context_management_phase_31` — agent must follow this rule to prevent context bloat.
