...
```json
{
  ...
  "canonical_references": {
    ...
    "lint_script_path": "./scripts/lint.sh (автономный lint, не блокирующий agent turn)"
  },
...
## 📄 Auto-update Index (Phase 4)

`scripts/rebuild-meta.sh` автоматически перестраивает `wiki/index.md` после каждого wiki edit:
- Извлекает H1-заголовки из всех страниц
- Формирует краткие описания (первые 2-3 предложения до первого ##)
- Группирует записи по категориям в приоритетном порядке: entities → concepts → comparisons → syntheses → overviews → notes → meetings → projects → bibliography → resources
- Добавляет timestamp и ссылку на Timeline

### Когда вызывается

| Process | Триггер |
|---------|----------|
| **Ingest** | После create_page (step 3a), update_existing (step 3b) — шаг `auto_update_index` |
| **Query** | Не вызывается (только при ingest/lint) |
| **Lint** | После link_validation_with_auto_fix (check_id=7), file_rename_or_delete (check_id=8) |

### Как использовать
```bash
# Вызывается автоматически из process-файлов
cd /path/to/loomana && ./scripts/rebuild-meta.sh --index-only
```
**Параметр `--index-only`**: перестраивает только index.md без повторного сканирования backlinks.json (быстрее).

### Логика генерации
1. **Парсинг страниц**: для каждой `.md` в wiki/:
   - Извлечь H1: `grep '^# ' page.md | head -1`
   - Извлечь первые 2-3 предложения до первого `##` после frontmatter
2. **Определение категории**: первая поддиректория (entities/, concepts/, comparisons/) или default к root wiki files (overview.md → overviews)
3. **Сортировка**: по названию страницы внутри каждой категории
4. **Формат записи**: `* [Название](путь) — краткое описание`
5. **Футер**: timestamp + ссылка на Timeline

### Почему это важно
- **Никакого ручного вмешательства**: index.md всегда актуален без ручной правки
- **Категории по приоритету релевантности**: syntheses/concepts/entities показываются раньше, чем notes/projects
- **Согласованность с registry.json и backlinks.json**: все meta-файлы обновляются из одного скрипта
