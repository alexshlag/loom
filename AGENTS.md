...
1. `grep -r "<old_path_or_name>" wiki/ --include="*.md"` — найти все файлы с ссылками на старый путь.
2. Для каждого найденного файла: заменить ссылку на новый путь.
3. Если target page не существует → `[!] Broken link: [text](path)` → предложить fix (create page / remove link).

**Обязательные триггеры:**
| Trigger | Description |
|---------|-------------|
| `page_created` | new wiki page — scan index.md, other entity/concept pages for mentions of the same topic |
