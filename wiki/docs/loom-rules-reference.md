...
type: documentation
category: docs
aliases: [rule-index, schema-ref-pattern, lazy-loading]
sources: [docs/rules-reference.md]
related: [wiki/docs/loom-getting-started.md, wiki/docs/loom-architecture.md, wiki/docs/loom-wiki-structure.md]
---
# Rules Reference Guide — All Technical Specs in One Place

# Rules Reference Guide — All Technical Specs in One Place

Page covering Rules Reference Guide — All Technical Specs in One Place — overview, usage patterns, and related resources.
## Overview

Rules live in `rules/*.json` as structured JSON schemas. They define conventions (naming, link format), algorithms (crosslink scoring, contradiction resolution), policies (source authority grading), and protocols (error handling).

> **Agent reads rules on demand** via schema_ref in process files. Never loads all rules at once — only when a process step references them. Example: `"rule": "rules/link_conventions.json#EXT-LINK-V1"` → agent fetches only that rule when writing a page.
...
| [`faq_summary.json`](../../rules/faq_summary.md) | Summary page creation triggers, lifecycle (decay/merge/update) | Before summary page op |
...
Auto-computed during ingest based on source authority. Three levels: `documented` (authoritative sources like official docs), `corroborated` (confirmed by 2+ independent sources), `assertion_only` (unconfirmed or from weak sources). Never set manually — only assigned by agent from source analysis.
...
### Path Guard Check (`path_guard_check.json`)
...
Explicitly lists directories/files that agents must not edit directly: `meta/*` — auto-generated, rebuilt by scripts; `tracking/*.json` (except raw_registry.json) — regenerated state files; system wiki pages — managed via process workflows only.
...
- [`wiki/docs/loom-architecture.md`](loom-architecture.md) — How rules fit into the layer architecture
- [`wiki/docs/loom-wiki-structure.md`](loom-wiki-structure.md) — Page creation workflow with rule references
- [`AGENTS.md`](../../AGENTS.md) — Canonical source for all rules
