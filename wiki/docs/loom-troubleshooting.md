...
Page covering Troubleshooting Guide — Common Issues & Fixes — overview, usage patterns, and related resources.
## Empty Wiki on Fresh Install

**Symptom:** You just cloned the repository and `wiki/` is empty (or nearly so). You expect content but see nothing.
...
See [`wiki/docs/loom-getting-started.md`](loom-getting-started.md) for the full onboarding flow.

---

## Source Ingest Fails with "Path Blocked"

**Symptom:** Agent reports a path is blocked or you get an error when trying to add a source.
...
### Solution
Provide the raw source file as-is, or paste content directly:
...
**Symptom:** Agent says a source is "already in the wiki" or "duplicate detected."
...
**Symptom:** Agent detects conflicting facts across wiki pages — e.g., one page says "version 2" and another says "version 3."

**Why:** Sources were added at different times, or from different authors with conflicting information.

### Resolution Process
The agent follows a priority cascade defined in `rules/contradiction_resolution.json`:
...
The agent proposes fixes with bidirectional callouts on both pages. You review and approve before any changes are made. No automatic overwrites happen without your go-ahead.

---

## Broken Links Detected During Lint

**Symptom:** `scripts/lint.sh` reports broken internal links — `[[wiki/page.md]]` pointing to non-existent files.
...
**Symptom:** `scripts/orphan-pages.sh` returns pages with zero incoming backlinks — they exist in wiki/ but nobody links to them.
...
**Symptom:** `scripts/rebuild-meta.sh` takes noticeably long on a large wiki (>500 pages).
...
**Symptom:** Agent runs commit but gets "nothing added to stage" or the staging mode is wrong (wiki vs dev).
...
See `rules/git_conventions.json` for the full convention. The agent reads this file before every commit operation to detect mode and apply correct staging rules.

---

## Agent Doesn't Read AGENTS.md at Session Start

**Symptom:** Agent behaves as if it doesn't know wiki conventions — creates pages in wrong directories, misses frontmatter fields, or doesn't follow naming rules.
...
**Symptom:** A script crashes or returns an error code instead of silently completing.
...
**Symptom:** working_memory.json or wiki/hot.md has stale data — agent remembers old focus topics, open pages from previous sessions, etc.
...
**Symptom:** You add sources from two different frameworks/projects that have overlapping concept names — e.g., "cache-system" could be generic, but also framework-specific. Agent creates one page where you expected two.
...
The exception list (truly abstract concepts without prefixes) is maintained in `rules/naming_conventions.json`. If you want something added there, propose it via [schema-patch].
...
|---------|---------------------|
| Empty wiki on fresh install | Add sources — see loom-getting-started.md |
...
- [`wiki/docs/loom-architecture.md`](loom-architecture.md) — How layers interact and where errors propagate
- [`rules/error_handling.json`](../../rules/error_handling.md) — The detect-analyze-resolve-continue protocol in detail
- [`wiki/docs/loom-scripts-guide.md`](loom-scripts-guide.md) — Every script provides usage info via --help flag
