...
category: concept
aliases: []
sources: ["docs/session-lifecycle.md"]
related: ["wiki/docs/loom-memory-hooks", "wiki/docs/loom-architecture", "rules/session_context_rules.json", "wiki/concepts/natural-memory"]
---
# Session Lifecycle Guide

# Session Lifecycle Guide

Page covering Session Lifecycle Guide — overview, usage patterns, and related resources.
## Overview: The Three-Phase Model

Every Loomana session follows three phases:
...
After bootstrap completes, the agent's working memory contains:
...
When multiple triggers fire simultaneously: **write ALL relevant sections in one pass** — not sequentially. Prevents partial writes and race conditions during compaction.
...
- Auto-distillation via `distill.sh --trajectory <path>` on capture complete
...
- `WM.current_mode = "lint"` (implicit, no explicit mode switch)
- Memory written after issues reported + fixes proposed
- Non-blocking: reports but never auto-applies without user approval
...
This is rare — both session memory AND long-term snapshot are gone. Usually caused by manual deletion or file corruption.
...
**Key distinction:** WM is the **operational memory** (current session, turn-to-turn), hot.md is the **continuity snapshot** (next session bootstrap). They share `focus_node` and `next_steps_todo` but serve different purposes:
- WM = "what am I doing RIGHT NOW"
- hot.md = "where was I when I last stopped?"
