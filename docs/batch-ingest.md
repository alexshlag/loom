...
This document describes how to process multiple sources efficiently through batch clustering — grouping related documents together before creating wiki pages. Use this when you have several articles, files, or URLs that cover overlapping topics and should be processed as a unified knowledge cluster rather than individually.
...
```python
# Build keyword index: {keyword → [source_files]}
for kw, sources in sorted(keyword_index.items(), key=lambda x: -len(x[1])):
    if len(sources) < 2: continue
    
    # Check if any source already in existing cluster (merge overlap)
    # Create new cluster only if no merge AND ≥2 sources share keyword
```
