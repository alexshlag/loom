...
This document defines the conventions for frontmatter schema, commit format, evidence grading, and tag patterns. These are the "rules of the road" — agent and human contributors should follow them consistently to maintain wiki quality and predictability.
...
> Evidence grade is stored in page metadata, not visible in frontmatter. Used internally for contradiction resolution priority cascade: documented > corroborated > assertion_only.
