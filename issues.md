...
|----------|----------|--------|
| Search Sufficiency | `grep_recursive_fallback` может генерировать шум | ✅ Решено: fallback только если COUNTER=0, результат ограничен MAX_RESULTS |
| Knowledge Hierarchy | Отсутствие четкой иерархии типов страниц при синтезе | ✅ Решено: issue A-D (Cross-role architecture), Compounding → Schema, Scoring → Query-specific |
| Novelty Threshold | Предложение новых страниц может привести к дублированию | ✅ Решено (см. ниже) |
| Source Weighting | Отсутствие оценки авторитетности источников | ⚠️ Issue #4 — требует отдельного обсуждения |
| Inline Citations | Технические инструкции в ответах не всегда имеют прямые ссылки | 🟡 Требует внимания (low priority, не блокирует работу) |
| Актуальность информации | Инструкция говорит "prefer newer version" при temporal conflict, но не учитывает устаревание фактов на новых страницах. В тесте: информация об Eureka Labs (2024) и Anthropic (2026) есть только в raw, но не интегрирована в wiki. | ✅ Решено (см. ниже) |
| Правила web_search | Не明确规定 когда можно/нельзя использовать web_search для проверки фактов из wiki. | ✅ Решено (см. ниже) |
| Критерий novel insight | Инструкция "If answer synthesizes from 2+ sources AND contains novel insight → flag for fixation" субъективна. Что считается insight? Сбор фактов или только новые выводы? | ✅ Решено (см. ниже) |
| Целостность cross-references | Инструкция требует добавлять backlinks к новым страницам, но не проверяет существующие страницы на сломанные ссылки. В тесте: `wiki/overview.md` упоминает Karpathy, но не проверена рабочая ссылка. | ✅ Решено (Lint check broken_links) |
...
| Авторитетные источники | В инструкции указано `official docs > community wiki > personal notes`, но нет механизма автоматического определения authoritative source. | ⚠️ Issue #4 — требует отдельного обсуждения |
...
## ✅ Решено: External Sources Update + Novelty Threshold (Issues #1-3) — 2026-06-26
...
| Режим | Триггер |
|-------|---------|
| **User-requested** | Пользователь явно просит «обнови X» или «проверь актуальность Y» |
| **Cron (default)** | Автоматически по расписанию (`check-new-sources.sh`) |

**Правило**: Агент никогда не делает `web_search` для обновления wiki без запроса. Только пользователь или cron.

### Issue #2: Когда `web_search` приоритетнее внутренних данных?

| Сценарий | Приоритет |
|----------|----------|
| **Тот же источник, но данные изменились** | ✅ Внешние данные приоритетны |
| **Разные источники противоречатся** | ⚠️ Перейти к Issue #4 (Authoritative Source Criteria) |
...
|-----|----------|----------|
| **Фактоидный синтез** | Сбор фактов из 2+ источников без нового вывода | ❌ Не создавать новую страницу |
| **Новый вывод** | Агент делает логический шаг, который не stated ни в одном источнике | ✅ Flag for fixation |
| **Контекстуальное применение** | Как применить концепты к текущему контексту | ⚠️ Notes/заметки, не новая wiki-страница |

**Правило**: Novel insight = новый логический вывод. Если цепочка "данные → вывод" прозрачна и все шаги из источников — это synthesis, не novel insight.

---

## ⚠️ Требает твоего решения (Issue #4 — Authoritative Sources)
...
**Варианты**:
1. **По домену**: `github.com/<org>/<repo>@<version>`, `*.com` (официальные сайты), `wikipedia.org`
...
*Created: 2026-06-24 | Status: Issues #1-3 Fixed ✅ (2026-06-26), Issue #4 Pending discussion | Last commit: ede2d60*
